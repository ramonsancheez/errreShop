@extends('layouts/layout')
    @section('main-content')
        <h1 class="product__title">Compra y vende cosas de segunda mano</h1>
        <h2 class="product__subtitle">casi, casi, sin moverte del sofá</h2>
        <section>
        ...
        <section>
            <h3 class="product__content-title">Todos los productos</h3>
            <ul class="product__list" id="store-products">
                @foreach($products as $product)
                    ...
                    @elseif ($product->user_id != Auth::id() and $product->is_active)
                            <li class="product">
                                <a href="/products/{{$product->id}}">
                                    <img class="product__image" src="{{$product->image_url}}" title="{{ $product->name }}" alt="{{ $product->name }}" width="200" height="200">
                                    <h2 class="product__name">{{ $product->name }}</h2>
        ...
        <section>
            ...
            <h3 class="product__content-title"> Algunas categorías</h3>
